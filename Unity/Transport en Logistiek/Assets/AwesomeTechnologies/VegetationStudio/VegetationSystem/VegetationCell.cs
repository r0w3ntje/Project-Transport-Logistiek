...
namespace AwesomeTechnologies
{
    public class VegetationCell : IHasRect
    {
        ...
        public List<Matrix4x4> DirectSpawnVegetation(string vegetationItemID, bool includePersistentStorage)
        {
            ...
            VegetationItemInfo vegetationItemInfo = CurrentvegetationPackage.GetVegetationInfo(vegetationItemID);
            ...
            if (vegetationItemInfo.VegetationType == VegetationType.Tree && vegetationItemInfo.UseCollisionDetection)
            {
                ...
                {                  
                    VegetationItemInfo treeVegetationItemInfo = CurrentvegetationPackage.VegetationInfoList[i];
        ...
        public void Setup(VegetationSettings vegetationSettings, VegetationPackage vegetationPackage, UnityTerrainData unityTerrainData)
        {
        ...
        private void CreateIndirectInfoList()
        {
            ...
            for (int i = 0; i <= CurrentvegetationPackage.VegetationInfoList.Count - 1; i++)
            {
                VegetationItemIndirectInfo vegetationItemIndirectInfo = new VegetationItemIndirectInfo(
                    ...
                    VegetationRenderType.InstancedIndirect && IsPlaying)
                {
        ...
        private void SpawnCellVegetation(int distanceBand)
        {
            ...
            for (int i = 0; i <= CurrentvegetationPackage.VegetationInfoList.Count - 1; i++)
            {
                ...
                if (distanceBand >= 3)
                {
                    if (CurrentvegetationPackage.VegetationInfoList[i].VegetationType != VegetationType.LargeObjects && CurrentvegetationPackage.VegetationInfoList[i].VegetationType != VegetationType.Tree)
                    {
                ...
                {
                    SpawnVegetation(CurrentvegetationPackage.VegetationInfoList[i], _currentvegetationSettings, matrixList, indirectMatrixList, IndirectInfoList[i]);
                }
        ...
        private void LoadPersistentVegetation(VegetationItemInfo vegetationItemInfo, List<Matrix4x4> matrixList, CustomList<Matrix4x4> indirectMatrixList, bool instancedIndirect)
        {
            ...
            for (int i = 0; i <= persistentVegetationInfo.VegetationItemList.Count - 1; i++)
            {
                ...
                if (vegetationItemInfo.UseVegetationMasksOnStorage)
                {
        ...
        private void SpawnVegetation(VegetationItemInfo vegetationItemInfo, VegetationSettings vegetationSettings, List<Matrix4x4> matrixList, CustomList<Matrix4x4> indirectMatrixList, VegetationItemIndirectInfo vegetationItemIndirectInfo, bool directSpawn = false, bool includePersistentStorage = true)
        {
            ...
            {
                LoadPersistentVegetation(vegetationItemInfo, matrixList, indirectMatrixList, vegetationItemInfo.VegetationRenderType == VegetationRenderType.InstancedIndirect && IsPlaying);
            }

            if (!vegetationItemInfo.EnableRuntimeSpawn)
            {
                if (vegetationItemIndirectInfo != null && vegetationItemIndirectInfo.InstancedIndirect && IsPlaying && includePersistentStorage)
                {
            ...
            for (int x = 0; x <= xSamples; x++)
            {
                for (int z = 0; z <= zSamples; z++)
                {
                    ...
                    if (vegetationItemInfo.ShaderType == VegetationShaderType.VegetationStudioGrass)
                    {
                    ...
                    {
                        if (vegetationItemIndirectInfo != null && vegetationItemIndirectInfo.InstancedIndirect && IsPlaying)
                        {
            ...
            if (vegetationItemIndirectInfo != null && vegetationItemIndirectInfo.InstancedIndirect && IsPlaying && !directSpawn)
            {
                vegetationItemIndirectInfo.AddMatrixList(indirectMatrixList);
            }
        }
